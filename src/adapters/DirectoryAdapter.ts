...
export function useFileAdapter(metadata: Pick<Directory.FileContent, 'id' | 'path'>) {
...
export function useFolderAdapter(metadata: Pick<Directory.FolderMetadata, 'id' | 'path'> = Directory.RootNode) {
  const dispatch: AppDispatch = useDispatch()
  const directoryState: DirectoryState = useReduxDirectoryState(dispatch)
  const localDirectoryDatabase = useLocalDirectoryDatabase(databaseId)
  console.log("use111111",metadata)
  ...
  const createFile = useMemo(() => (params: Pick<File.createFileParams, 'name' | 'path'>) => {
  ...
  const createFolder = useMemo(() => (params: Pick<File.createFileParams, 'name' | 'path'>) => {
    Folder.createFolder({
      name: params.name,
      parentId: metadata.id,
      path: params. path
