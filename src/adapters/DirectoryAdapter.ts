...
export function useFileAdapter(
  metadata: Pick<Directory.FileContent, "id" | "path">
) {
  ...
  const updateContent = useMemo(
    () => (newContent: Directory.FileContent["content"]) => {
      File.saveFile(
        {
          ...fileMetadata,
          content: newContent,
  ...
  const renameFile = useMemo(
    () => (newName: Directory.FileMetadata["name"]) => {
...
export function useFolderAdapter(
  metadata: Pick<Directory.FolderMetadata, "id" | "path"> = Directory.RootNode
) {
  const dispatch: AppDispatch = useDispatch();
  const directoryState: DirectoryState = useReduxDirectoryState(dispatch);
  const localDirectoryDatabase = useLocalDirectoryDatabase(databaseId);
  ...
  const createFile = useMemo(
    () => (params: Pick<File.createFileParams, "name" | "path">) => {
  ...
  const renameFolder = useMemo(
    () => (newName: Directory.FolderMetadata["name"]) => {
