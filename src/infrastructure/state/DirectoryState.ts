...
interface ExplorerState {
  ...
  activeFolder: Directory.FolderMetadata;
...
const initialState: ExplorerState = {
  ...
  activeFolder: Directory.RootNode,
  fileContent: {},
  fileStatus: {},
  folderStatus: {
...
const reduxDirectoryState = createSlice({
  ...
  reducers: {
    setFolderMetadata(
      state,
      { payload }: PayloadAction<Directory.FolderMetadata>
    ) {
      state.folderMetadata[payload.id] = payload;
    },
    setActiveFolderMetadata(
      state,
      { payload }: PayloadAction<Directory.FolderMetadata>
    ) {
    ...
    setFileStatus(
      ...
      {
        payload,
      }: PayloadAction<{ file: Pick<Directory.Node, "id">; status: FileStatus }>
    ) {
    ...
    deleteFolderMetadata(
      state,
      { payload }: PayloadAction<Pick<Directory.Node, "id">>
    ) {
...
class ReduxDirectoryState implements DirectoryState {
  ...
  setActiveFolderMetadata(folder: Directory.FolderMetadata): void {
    this.dispatch(reduxDirectoryState.actions.setActiveFolderMetadata(folder));
  }
  ...
  setFolderStatus(
    ...
    this.dispatch(
      reduxDirectoryState.actions.setFolderStatus({ folder, status })
    );
  }
...
export const selectFolderContent = (
  folderMetadata: Pick<Directory.FolderMetadata, "id">
) => {
...
export const selectActiveFolderMetadata = () => {
  return (state: RootState): Directory.FolderMetadata => {
    return state.directory.activeFolder;
  };
};
export const selectFolderMetadata = (
  folderMetadata: Pick<Directory.FolderMetadata, "id">
) => {
...
export const selectFileMetadata = (
  fileMetadata: Pick<Directory.FileMetadata, "id">
) => {
  return (state: RootState) => {
...
export const selectFileContent = (
  fileMetadata: Pick<Directory.FileMetadata, "id">
) => {
...
export const selectFolderStatus = (
  folderMetadata: Pick<Directory.FolderMetadata, "id">
) => {
  return (state: RootState) => {
...
export const selectAnsestors = (
  folderMetadata: Pick<Directory.FolderMetadata, "id">
) => {
  return (state: RootState) => {
    let currentNode: Directory.FolderMetadata | undefined =
      state.directory.folderMetadata[folderMetadata.id];
    const ansestors = [];
